/* =========================================================
   STEPHEN CURRY · THE SHOOTER'S REALM — 交互引擎
   ========================================================= */
(function () {
  "use strict";
  ...
  (function draw() {
    ...
    for (const p of parts) {
      ...
      ctx.shadowColor = "rgba(255,199,44,.8)";
  ...
  newsWrap.innerHTML = items.map((n, i) => {
    if (i === 0) {
      ...
      <article class="news-card hero-card reveal">
        ...
        <div class="pic"><img src="assets/img/olympics-trio.jpg" alt="库里与勒布朗、杜兰特" loading="lazy"></div>
      </article>`;
    }
    return `
  ...
  const chartKeys = {
    pts: { label: "场均得分", color: "#FFC72C", max: 35 },
    threepm: { label: "场均三分", color: "#9ed3ff", max: 450 },
    ast: { label: "场均助攻", color: "#b7f5c8", max: 12 },
    gp: { label: "出场数", color: "#ffd27f", max: 90 },
  };
  const TEAM_NAME = { GSW: "金州勇士" };
  let chartKey = "pts";
  let chartProg = 0;
  let hoverIdx = -1;
  const seasons = CURRY.seasons;
  ...
  function drawChart() {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      cctx.fillText(val.toFixed(0), padL - 8 * dpr, y + 4 * dpr);
    }
    ...
    seasons.forEach((s, i) => {
      if (s.champ || s.mvp) {
        cctx.fillStyle = s.champ ? "rgba(255,199,44,.10)" : "rgba(29,66,138,.22)";
    ...
    seasons.forEach((s, i) => {
      ...
      g.addColorStop(1, "rgba(255,199,44,.05)");
    ...
    cctx.fillStyle = "rgba(255,199,44,.7)";
    cctx.fillText("金底 = 夺冠赛季 · 蓝底 = MVP 赛季 · 悬停查看详情", padL + cw / 2, h - 10 * dpr);
    ...
    if (hoverIdx >= 0) {
      ...
      const lines = [
        `${s.year} · ${TEAM_NAME[s.team] || s.team}`,
        `得分 ${s.pts}  三分 ${s.threepm}  助攻 ${s.ast}`,
      ...
      cctx.strokeStyle = "rgba(255,199,44,.5)";
      cctx.beginPath(); cctx.roundRect(bx, by, tw, th, 8 * dpr); cctx.fill(); cctx.stroke();
      cctx.textAlign = "left";
      lines.forEach((l, li) => {
        cctx.fillStyle = li === 0 ? "#FFC72C" : "rgba(242,240,234,.88)";
  ...
  tbody.innerHTML = seasons.map((s) => `
    <tr>
      <td class="yr">${s.year}
        ...
        ${s.champ ? '<span class="pill champ">🏆 总冠军</span>' : ""}
        ${s.fmvp ? '<span class="pill champ">FMVP</span>' : ""}
        ${s.note ? `<span class="tnote">${s.note}</span>` : ""}
      </td>
      <td>${s.gp}</td><td><b>${s.pts}</b></td><td>${s.ast}</td><td>${s.threepm}</td>
      <td>${s.fg ? s.fg + "%" : "—"}</td><td>${s.tp ? s.tp + "%" : "—"}</td>
    </tr>`).join("");

  /* ---------- 纪录 ---------- */
  $("#recordsGrid").innerHTML = CURRY.records.map((r) => `
    <div class="record-card reveal">
      <div class="record-num">${r.stat}</div>
      <div class="record-rank">${r.rank}</div>
  ...
  tlTrack.innerHTML = CURRY.timeline.map((t) => `
  ...
  function renderStint(i) {
    const s = CURRY.stints[i];
    stintWrap.innerHTML = `
      <div class="stint-logo-box" style="--stint-glow:${s.color}33">
        <img src="assets/img/logo_warriors.svg" alt="${s.team}">
      </div>
      <div>
        <div class="stint-years">${s.years}</div>
        <h3 class="stint-name">${s.team}</h3>
        <div class="stint-head">${s.head}</div>
        <p class="stint-desc">${s.summary}</p>
        <div class="stint-stats">
          ${s.stats.map((x) => `· ${x.k}：<b>${x.v}</b>`).join("<br>")}
        </div>
        <p class="stint-line">${s.line}</p>
      </div>`;
  ...
  renderStint(1); // 默认展示王朝 2014–2019

  /* ---------- 荣誉 ---------- */
  $("#honorGrid").innerHTML = CURRY.honors.map((h) => `
    <div class="honor-card reveal">
      <div class="honor-cat">${h.cat}</div>
      <div class="honor-list">
        ${h.items.map((it) => `<div class="honor-item">${it}</div>`).join("")}
      </div>
    </div>`).join("");
  ...
  $("#offGrid").innerHTML = `
    <div class="off-banner reveal">
      <img src="assets/img/olympics-flag.jpg" alt="库里身披美国国旗">
      <div class="cap">
        <h3>More Than A Shooter</h3>
        <p>从戴维森的小个子到改变篮球运动的人，从 Curry Brand 到 Eat. Learn. Play. 基金会——库里用十七年证明：他不只是一名射手。2024 年巴黎奥运夺金后，他与妻子 Ayesha 在奥克兰的公益版图持续扩大，SC30 与 Unanimous Media 也把他的影响力延伸到影视与内容创业。</p>
      </div>
    </div>` +
    CURRY.offcourt.map((o) => `
  ...
  galTrack.innerHTML = CURRY.gallery.map((g, i) => `
  ...
  let lbIdx = 0;
  function openLb(i) {
    lbIdx = (i + CURRY.gallery.length) % CURRY.gallery.length;
    const g = CURRY.gallery[lbIdx];
  ...
  let qIdx = 0, qTimer;
  qDots.innerHTML = CURRY.quotes.map((_, i) => `<i class="${i === 0 ? "on" : ""}"></i>`).join("");
  function showQuote(i) {
    qIdx = i;
    $$("#quoteDots i").forEach((d, di) => d.classList.toggle("on", di === i));
    const q = CURRY.quotes[i];
    gsap.to([qText, qEn, qFrom], {
      opacity: 0, y: -16, duration: 0.4, ease: "power2.in",
      onComplete: () => {
        qText.textContent = q.cn; qEn.textContent = q.en; qFrom.textContent = "—— " + q.from;
        gsap.fromTo([qText, qEn, qFrom], { opacity: 0, y: 26 }, { opacity: 1, y: 0, stagger: 0.1, duration: 0.7, ease: "power3.out" });
      }
    });
  }
  function qAuto() { clearInterval(qTimer); qTimer = setInterval(() => showQuote((qIdx + 1) % CURRY.quotes.length), 6000); }
